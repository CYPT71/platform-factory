...
import (
	...
	v1alpha1 "github.com/CYPT71/secure-oci-base/api/v1alpha1"
	v1beta1 "github.com/CYPT71/secure-oci-base/api/v1beta1"
)
...
func Decode(input io.Reader) (v1.Pipeline, Graph, error) {
	data, err := io.ReadAll(io.LimitReader(input, maxDefinitionBytes+1))
	if err != nil {
		return v1.Pipeline{}, Graph{}, fmt.Errorf("read pipeline: %w", err)
	}
	if len(data) > maxDefinitionBytes {
		return v1.Pipeline{}, Graph{}, errors.New("pipeline exceeds the 1 MiB definition limit")
	}
	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	var definition v1.Pipeline
	if err := decoder.Decode(&definition); err != nil {
		return v1.Pipeline{}, Graph{}, fmt.Errorf("decode pipeline: %w", err)
	}
	if err := decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return v1.Pipeline{}, Graph{}, errors.New("pipeline must contain exactly one JSON object")
	}
...
var acceptedVersions = map[string]bool{v1.APIVersion: true, v1.LegacyAPIVersion: true, v1beta1.APIVersion: true, v1beta1.LegacyAPIVersion: true, v1alpha1.APIVersion: true, v1alpha1.LegacyAPIVersion: true}

func Analyze(definition v1.Pipeline) (Graph, error) {
	var issues []Issue
	...
	stages := make(map[string]v1.Stage, len(definition.Stages))
...
func Fingerprint(definition v1.Pipeline) (string, error) {
