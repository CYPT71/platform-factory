...
func TestMigrationPlanSemanticValidation(t *testing.T) {
	tests := map[string]func(*MigrationPlan){
		...
		"secret value":          func(p *MigrationPlan) { p.Resources[0].Attributes["description"] = "password=sentinel" },
...
func TestMarshalYAMLRefusesSecretPlan(t *testing.T) {
	p := validCanonicalPlan(t)
	p.Resources[0].Attributes["description"] = "api_key=sentinel"
	if _, err := MarshalYAML(&p); err == nil {
		t.Fatal("secret plan serialized")
	}
}
