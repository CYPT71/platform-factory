...
// (platform-factory publish --policy/--evidence, verify-release --policy/
// --evidence); Evaluate must never panic on adversarial input, only
// return an error or a Decision.
func FuzzEvaluate(f *testing.F) {
	f.Add(`{"api_version":"platform-factory.dev/policy/v1","require_sbom":true}`, `{"sbom":true,"subject_digest":"sha256:00"}`)
	f.Add(`{"api_version":"wrong"}`, `{}`)
	f.Add(`{}`, `{}`)
	f.Add(`null`, `null`)
	f.Add(`{"api_version":"platform-factory.dev/policy/v1","require_pins":true,"require_hardening":true,"require_sbom":true,"require_provenance":true,"require_signature":true,"require_reproducible":true}`, `{}`)
