...
// decoder - the input to `platform-factory publish --journal PATH` and to
...
func FuzzFromJournal(f *testing.F) {
	...
	f.Add(`{"api_version":"platform-factory.dev/journal/v1","pipeline_fingerprint":"x","password":"leaked"}`)
	f.Add(`{}`)
	f.Add(`null`)
	f.Add(``)
	f.Add(`{"api_version":"platform-factory.dev/journal/v1","pipeline_fingerprint":"x","stages":[[[[[[[[[[]]]]]]]]]]]}`)
	...
	deep := `{"api_version":"platform-factory.dev/journal/v1","pipeline_fingerprint":"x","stages":` +
	...
	f.Fuzz(func(t *testing.T, journal string) {
		...
		_, _ = FromJournal(strings.NewReader(journal), "https://platform-factory.dev/builder/v1")
	})
}
