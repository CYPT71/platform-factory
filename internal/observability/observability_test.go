...
func TestRedactValueHandlesEveryInputShape(t *testing.T) {
	if got := redactValue(Fields{"password": "hunter2", "ok": "fine"}); got.(map[string]any)["password"] != redactedValue {
		t.Fatalf("Fields case: got=%v", got)
	}
	if got := redactValue(map[string]any{"secret": "x"}); got.(map[string]any)["secret"] != redactedValue {
		t.Fatalf("map[string]any case: got=%v", got)
	}
	if got := redactValue(map[string]string{"token": "abc", "name": "ok"}); got.(map[string]any)["token"] != redactedValue || got.(map[string]any)["name"] != "ok" {
		t.Fatalf("map[string]string case: got=%v", got)
	}
	if got := redactValue([]any{"https://user:pass@example.com/", 42}); got.([]any)[1] != 42 {
		t.Fatalf("[]any case: got=%v", got)
	} else if !strings.Contains(got.([]any)[0].(string), "redacted") {
		t.Fatalf("[]any case did not redact nested URL: %v", got)
	}
	if got := redactValue("https://user:pass@example.com/path"); !strings.Contains(got.(string), "redacted") {
		t.Fatalf("string case: got=%v", got)
	}
	if got := redactValue(errors.New("https://user:pass@example.com/path")); !strings.Contains(got.(string), "redacted") {
		t.Fatalf("error case: got=%v", got)
	}
	if got := redactValue([]int{1, 2, 3}); len(got.([]any)) != 3 || got.([]any)[0] != 1 {
		t.Fatalf("reflect slice case: got=%v", got)
	}
	if got := redactValue(42); got != 42 {
		t.Fatalf("passthrough case: got=%v", got)
	}
}
...
func TestSetDefaultReplacesTheGlobalLogger(t *testing.T) {
	defer DeferResetGlobalLogger()()
	replacement := newDefaultLogger()
	replacement.SetLevel(LevelError)
	SetDefault(replacement)
	if Default().GetLevel() != LevelError {
		t.Fatalf("expected SetDefault to replace the global logger")
	}
}

func TestPackageLevelLogFunctionsCoverEveryLevel(t *testing.T) {
	defer DeferResetGlobalLogger()()
	SetGlobalLevel(LevelDebug)
	var buf bytes.Buffer
	SetGlobalOutput(&buf)
	...
	output := buf.String()
	for _, want := range []string{"debug message", "warn message", "dpanic message", "panic message"} {
...
func TestTracerStartSpanMethodAndCurrentSpan(t *testing.T) {
	tracer := newDefaultTracer(newDefaultLogger())
	span := tracer.StartSpan("method-span")
	if span == nil || span.Name != "method-span" {
		t.Fatalf("span=%+v", span)
	}

	// CurrentSpan with no context values returns nil.
	if got := tracer.CurrentSpan(context.Background()); got != nil {
		t.Fatalf("expected nil for a context with no span, got %+v", got)
	}
	if got := tracer.CurrentSpan(nil); got != nil { //nolint:staticcheck // exercising the explicit nil-context guard
		t.Fatalf("expected nil for a nil context, got %+v", got)
	}

	rootSpan, ctx := tracer.StartSpanWithContext(context.Background(), "root")
	if current := tracer.CurrentSpan(ctx); current != rootSpan {
		t.Fatalf("expected CurrentSpan to return the active root span, got %+v", current)
	}
}

func TestSetDefaultTracerReplacesTheGlobalTracer(t *testing.T) {
	...
	if DefaultTracer() != replacement {
		t.Fatal("expected SetDefaultTracer to replace the global tracer")
	}
}
