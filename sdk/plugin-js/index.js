...
class Server {
  ...
  async _dispatch(request) {
    ...
    if (!handler) {
      return { id, error: { code: 404, message: `unknown method ${JSON.stringify(method)}` } };
    }
    try {
      ...
      return { id, error: { code: 500, message: String((err && err.message) || err) } };
    }
  }
}
...
module.exports = { Server, RPCError, writeMessage, CONTENT_TYPE, LEGACY_CONTENT_TYPE, PROTOCOL_VERSION };
