...
Build policy can be enforced before publication. Low-level builds accept
`--policy policies/build.json --reports reports`; requirements for SBOM,
provenance or signature additionally require the corresponding `--dist` and
`--sign-key-dir` inputs. Reproducibility policy requires `--rebuild=2` (or
more) together with `--require-identical`. Project builds automatically load
`policies/build.json`; a signature requirement uses the pre-existing,
non-symlink `.platform-factory/keys/release.key`. Keys are never generated by
policy preflight. Requirements the current build cannot prove—host toolchain
pins or runtime hardening in the v1 project schema—fail before any build
command or output instead of being reported as satisfied. The final policy
decision is written only after the requested evidence files exist.
...
The native MicroVM packager is also the runtime-contract boundary. It verifies
the selected OCI manifest and every layer, converts `Entrypoint` + `Cmd`,
`Env`, `WorkingDir`, and numeric `User` into
`/etc/platform-factory/process.json`, then creates the deterministic
initramfs. Both the native KVM/HVF path and the QEMU/KubeVirt preparation
scripts use this same implementation. Declared OCI ports must have matching
`--publish` forwards for native execution; OCI volumes fail until explicit
host sources are provided. `Healthcheck`, `StopSignal`, `Shell`, and `OnBuild`
are rejected instead of being silently dropped. The packager reports required
ports and volume targets in its JSON result for the later MachineSpec step.
...
One build may also expose the same content under several image names or tags
without rebuilding or duplicating blobs:

```bash
pf build --image example/api --tag v1 \
  --reference example/api:stable \
  --reference example/compat-api:2026.08 ./service
```

Every reference is validated before output creation. The resulting OCI index
contains one descriptor per reference and platform, all pointing at the same
content-addressed manifest for that platform. Duplicate, digest-based,
untagged, malformed, or excessive alias sets are refused.
...
### X.509-pinned release verification

`pf verify-release` can trust a DSSE signer through an explicitly pinned
certificate chain instead of a raw `--trusted-key`:

```bash
pf verify-release \
  --certificate release-leaf.pem \
  --intermediate-certificate issuing-ca.pem \
  --root-certificate offline-root.pem \
  --signature dist/signature.dsse.json \
  --provenance dist/provenance.dsse.json \
  --sbom dist/sbom.json --policy policies/release.json \
  --evidence reports/evidence.json dist/oci-layout
```

Each certificate file must contain exactly one PEM certificate and be a
regular, non-symlink file. Roots are never inferred from the envelope or the
host trust store: at least one `--root-certificate` is mandatory. Only after
the complete chain validates is the leaf's Ed25519 key allowed to verify the
DSSE subject and provenance. Intermediate and root flags are repeatable.

To bind provenance to the build that actually ran, also pass the independent
pipeline journal:

```bash
pf verify-release --provenance provenance.json --journal journal.json \
  --builder-id https://platform-factory.dev/builder/v1 [OTHER OPTIONS] dist/oci-layout
```

The verifier regenerates the SLSA v1 predicate from the bounded, secret-safe
journal parser and requires structural equality with the verified provenance
payload. A correctly signed predicate from a different invocation is rejected.

`pf publish --attestation FILE --sign` accepts a repeatable collection of
external predicates using `schemas/external-attestation-v1.schema.json`. Each
document is validated strictly before any upload, revalidated before tag
movement, bound to the digest actually installed in the registry, wrapped as
an in-toto Statement v1, signed with DSSE/Ed25519, and pushed as an OCI subject
artifact. Unsigned, symlinked, oversized, malformed, or unknown-field inputs
are refused.

### MicroVM boot bundle output

For a generated project with `isolation: microvm`, `pf project build` uses its
v2 lock pins for both kernel and initramfs, then writes
`dist/boot-bundle/bundle.json`. The canonical bundle binds those two verified
digests to the exact OCI rootfs/manifest digest produced by the build, together
with its platform and image reference. Unsafe or symlinked `dist` directories
are refused.

Hand-written legacy projects without a lock (or with the compatibility-only v1
lock) still build, but deliberately produce no boot bundle because they cannot
prove the boot inputs.

### Contextual platform capability preflight

`pf plan`, `pf project build --dry-run`, and the project build result expose a
structured `capabilities` assessment. Build-time requirements are enforced
before any custom command or output mutation: the OCI target must be supported
and an interpreted profile must point at a real runtime file. Capabilities only
needed later are still probed and reported, but do not make portable packaging
depend on the local host: Docker/Podman for container execution, and the native
hypervisor plus VMM sandbox facilities for MicroVM execution. The actual
`pf run` paths retain their runtime/native-backend checks when those deferred
capabilities are consumed.
...
Publication records are write-once. A normal retry never guesses whether a
previous process moved a tag: it reports the failed or indeterminate operation
identifier and stops. After checking that the earlier process is no longer
running, reconcile it with a unique operator token:

```bash
pf publish --yes --recover-operation incident-2026-08-21-01 \
  --sbom --sign --provenance provenance.json \
  --policy policy.json --evidence evidence.json \
  oci-image registry.example/team/app:v1
```

The recovery is a new atomically claimed attempt bound to the exact verified
manifest digest and registry target. It repeats the native digest-addressed
push, remote digest verification, evidence publication and policy evaluation;
only then does it install the tag. Reusing the same recovery token does not
repeat a completed mutation. If that attempt is itself interrupted, use a new
token after observing the previous process. Tokens are identifiers, not
secrets; never put credentials in them.

Provider authentication, routing differences and the evidence required before
a provider is called tested are documented in
[Registry provider compatibility](registry-providers.md).
...
The initramfs builder emits a small JSON requirements document containing the
verified OCI manifest/rootfs digests plus declared ports and volumes. Turn it
into a content-addressed native VMM contract without guessing host resources:

```bash
pf microvm machine-spec \
  --name demo \
  --requirements initramfs-result.json \
  --kernel-digest sha256:... \
  --initramfs-digest sha256:... \
  --publish 18080:8080/tcp \
  --volume-source ro@/data=/absolute/host/data \
  --dns 1.1.1.1
```

Every OCI `ExposedPort` and `Volume` must have an explicit mapping. Requirements
files are size-bounded, decoded strictly, and cannot be symlinks; volume sources
must be existing absolute non-symlink files or directories. The output binds
the OCI manifest, rootfs, kernel and initramfs digests in `MachineSpec.Bundle`.
It produces the backend-neutral contract; a backend that cannot attach the
requested volume or DNS configuration still refuses it rather than silently
dropping it.
