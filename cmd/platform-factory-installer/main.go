...
// CI and cross-compilation. Run it from within the platform-factory repo:
//
...
func findRepoRoot() (string, error) {
	...
	if gomod == "" || filepath.Base(gomod) != "go.mod" {
		return "", errors.New("run the installer from within the platform-factory repository (go.mod not found)")
	}
