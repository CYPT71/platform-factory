...
func TestProvisionRuntimeFromRootRequiresALoadedPlugin(t *testing.T) {
	...
	if _, err := provisionRuntimeFromRoot(loaded, "python", t.TempDir(), execute, &stderr); err == nil {
...
func TestProvisionRuntimeFromRootSurfacesExecuteError(t *testing.T) {
	loadFakeLanguagePlugin(t, "python")
	loaded := loadRuntimeTestProject(t)
	execute := func(string, []string, string, io.Writer, io.Writer) error {
		return errors.New("boom")
	}
	var stderr bytes.Buffer
	_, err := provisionRuntimeFromRoot(loaded, "python", t.TempDir(), execute, &stderr)
...
func TestProvisionRuntimeFromRootSucceeds(t *testing.T) {
	...
	var stderr bytes.Buffer
	manifest, err := provisionRuntimeFromRoot(loaded, "python", "/image-root", execute, &stderr)
	if err != nil {
...
func TestRunPluginProvisionRuntimeRequiresLanguageAndImage(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		stdout.Reset()
		stderr.Reset()
		if code := runPluginProvisionRuntime(context.Background(), args, &stdout, &stderr, execute); code != 2 {
...
func TestRunPluginProvisionRuntimeRejectsProjectWithExistingRuntime(t *testing.T) {
	...
	execute := func(string, []string, string, io.Writer, io.Writer) error { return nil }
	var stdout, stderr bytes.Buffer
	code := runPluginProvisionRuntime(context.Background(), []string{"--language", "python", "--image", "python@sha256:abc", "--dir", dir}, &stdout, &stderr, execute)
	if code != 2 {
...
func TestRunPluginProvisionRuntimeRejectsUndiscoverableProject(t *testing.T) {
	execute := func(string, []string, string, io.Writer, io.Writer) error { return nil }
	var stdout, stderr bytes.Buffer
	code := runPluginProvisionRuntime(context.Background(), []string{"--language", "python", "--image", "python@sha256:abc", "--dir", t.TempDir()}, &stdout, &stderr, execute)
	if code != 2 {
		t.Fatalf("code=%d stderr=%s", code, stderr.String())
	}
}
